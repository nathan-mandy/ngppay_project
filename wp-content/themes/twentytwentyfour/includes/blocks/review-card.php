...
class Review_Post_Card extends Skeletor_Block { 
    ...
    public static $block_settings = [
     ...
     public static function before_render($block_data) {
        ...
        $block_data['category'] = self::get_post_category($post);
        $block_data['title'] = get_the_title($post);
        $block_data['featured_image'] = get_the_post_thumbnail_url($post);  // Get featured image
        $block_data['image'] = 'http://ngppayproject.local/wp-content/uploads/2024/07/reviewauthor.png'; // Replace with actual path to your image
